...
class SetCenterMapOnUserOnStartUseCaseTest {
    ...
    fun setUp() {
        ...
        setCenterMapOnUserOnStartUseCase = SetCenterMapOnUserOnStartUseCaseImpl(settingsRepository, testDispatcher)
    }

    @Test
    fun `invoke with true should save settings with centerMapOnUserOnStart as true`() = runTest(testDispatcher) {
        // Given
        ...
        // When
        setCenterMapOnUserOnStartUseCase(enabled = true)
        ...
        assertEquals(true, captured?.value?.centerMapOnUserOnStart)
    }

    @Test
    fun `invoke with false should save settings with centerMapOnUserOnStart as false`() = runTest(testDispatcher) {
        // Given
        val initialSettings = Settings(
            servers = emptyList(),
            refreshInterval = 5,
        ...
        // When
        setCenterMapOnUserOnStartUseCase(enabled = false)

        // Then
        verifySuspend(mode = VerifyMode.exactly(1)) { settingsRepository.saveSettings(any()) }
        val captured = settingsSlot.value as? SlotCapture.Value.Present
        assertEquals(false, captured?.value?.centerMapOnUserOnStart)
    }
}
